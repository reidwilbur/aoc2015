...
spec = do
  describe "Day16" $ do
    context "parseSue" $ do
      it "should parse a Sue" $ do
        parseSue "Sue 1: goldfish: 6, trees: 9, akitas: 0" `shouldBe` Sue "Sue 1" (Map.fromList [("goldfish",6),("trees",9),("akitas",0)])

    context "similarity" $ do
      it "should have similarity of 1 for same maps" $ do
        let m1 = Map.fromList [("k1", 0),("k1", 1),("k2", 2),("k3", 3)]
        similarity m1 m1 `shouldBe` 1.0

      it "should work for maps with missing dimensions" $ do
        let m1 = Map.fromList [("k1", 0),("k1", 1),("k2", 2),("k3", 3)]
        let m2 = Map.fromList [("k1", 1),("k2", 2)]
        similarity m1 m2 `shouldSatisfy` (\v -> within v 0.999 0.001)

      it "should have similarity of -1 for opposite maps" $ do
        let m1 = Map.fromList [("k1", 0),("k1", 1),("k2", 2),("k3", 3)]
        let m2 = Map.fromList [("k1", 0),("k1", -1),("k2", -2),("k3", -3)]
        similarity m1 m2 `shouldBe` (-1.0)

    context "getBestMatch" $ do
      it "should return Sue 103 for day16TestInput" $ do
