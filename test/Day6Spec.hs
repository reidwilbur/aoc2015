  ...
  describe "Day6" $ do
    ...
    context "parseInstr" $ do
      it "should return Toggle (461,550) (564,900) for 'toggle 461,550 through 564,900'" $ do
        parseInstr "toggle 461,550 through 564,900" `shouldBe` (Toggle (461,550) (564,900))

      it "should return TurnOff (461,550) (564,900) for 'turn off 461,550 through 564,900'" $ do
        parseInstr "turn off 461,550 through 564,900" `shouldBe` (TurnOff (461,550) (564,900))

      it "should return TurnOn (461,550) (564,900) for 'turn on 461,550 through 564,900'" $ do
        parseInstr "turn on 461,550 through 564,900" `shouldBe` (TurnOn (461,550) (564,900))

    context "execInstr" $ do
      it "should turn on all addrs for TurnOn (0,0) (2,2)" $ do
        execInstr Set.empty (TurnOn (0,0) (2,2)) `shouldBe` Set.fromList [ (r,c) | r <- [0..2], c <- [0..2]]

      it "should turn off all addrs for TurnOn (0,0) (2,2)" $ do
        let s = Set.fromList [ (r,c) | r <- [0..2], c <- [0..2]] in 
          execInstr s (TurnOff (0,0) (2,2)) `shouldBe` Set.empty

      -- diagram of lights for test below
      -- before after
      -- _**    *_*
      -- ***    __*
      -- ***    ***
